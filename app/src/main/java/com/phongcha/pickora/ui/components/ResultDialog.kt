...
fun ResultDialog(
    ...
    val interstitialManager = remember {
        try {
            org.koin.java.KoinJavaComponent.getKoin().get<InterstitialAdManager>()
        } catch (_: Exception) {
            null
        }
    }

    // Record this pick for interstitial counter
    LaunchedEffect(Unit) {
        interstitialManager?.recordPick()
    }

    Dialog(
        onDismissRequest = {
            onDismiss()
            // Show interstitial when user dismisses the result
            if (activity != null) {
